...
public class Edit extends BaseActivity implements OnClickListener{
    ...
    String Author="";
    String imgP;
    MyDataBaseHelper dbHelper = new MyDataBaseHelper(this,"Diary.db",
            null,1);
    ...
    private void InitNote() {       //进行数据填装
        ...
        Cursor cursor  = db.query("Diary",new String[]{"id","title","author",
                        "content","imgP"},
                "id=?",new String[]{tran+""},null,null,
                null);
        ...
        switch (v.getId()){
            ...
            case R.id.ButtonSave:       //保存该界面的数据
                ...
                if(Title.length()==0){
                    ...
                    db1.update("Diary", values, "id=?",
                            new String[]{tran + ""});        //对数据进行更新
                    Edit.this.setResult(RESULT_OK, getIntent());
                    Edit.this.finish();
                }
